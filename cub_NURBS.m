function [xyw,res,Z,Zin,cmom,bbox,itlevel] = cub_NURBS(ade,structure_RS,...
...
%
% Z  : all analysed points from which only Zin are used in compression;
%
...
[~,on,sp_xnV,sp_xdV,sp_ynV,sp_ydV,boxVx,singular_points_X,...
    singular_points_Y]=indomain_NURBS([],structure_RS,Nbox,safe_mode);


% Compute Tens.Chebyshev-Vandermonde moments.
[cmom,bbox]=chebmom(ade,sp_xnV,sp_xdV,sp_ynV,sp_ydV);
...
for itlevel=1:Lmax
    ...
    x=linspace(bbox(1),bbox(2),k); y=linspace(bbox(3),bbox(4),k);
    [u,v]=meshgrid(x,y); ZL=[u(:) v(:)];

    % Find points of ZL in the domain and store them in "X".
    [inside,on]=indomain_NURBS(ZL,structure_RS,Nbox,safe_mode,...
    ...
    if length(X) == 0
        ...
        if (extraction_type == 1) | (extraction_type == 2) | ...
                (extraction_type == 3)
...
res=norm(V(ind1,:)'*w-cmom);
if res>restol | length(w) == 0
    fprintf(2,'\n \t Tried backslash (NI rule): fail. Mom. error: %1.3e',...
        res);
else

    ineg=length(find(w < 0));
    fprintf(2,'\n \t Tried backslash (NI rule): OK. Mom. error: %1.3e ',...
        res);
...
function [cmom,bbox] = chebmom(ade,sp_xnV,sp_xdV,sp_ynV,sp_ydV)
